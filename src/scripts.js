//TODO - add comments about firebase

// setting some constants for future callbacks to enable targeting of functions
...
headerRef && headerRef.insertAdjacentHTML('beforebegin', headerEl) //adds html code when called on
navRef && firebaseApp.auth().onAuthStateChanged(user => { //checks if the user is logged in
    if (user) {
        ...
        navRef.insertAdjacentHTML('beforeend', navEl('<li class="nav-item px-lg-4"><a class="nav-link text-uppercase" href="login.html">Login</a></li>')) // shows if the user is logged out
    }
})

firebaseApp.auth().onAuthStateChanged(user => {
    if (user) {
        console.log('logged in: ', user) // gives a reply when you login
    } {
        console.log('logged out') // gives a reply when you logout
    }
})